...
export function WatchlistPanel() {
  ...
  // Add stock search (from main search dropdown)
  ...
  // Debounced market search from main search input
  useEffect(() => {
    if (!search.trim()) { setAddResults([]); return }
    ...
    <>
      <div className="panel-header">
        ...
        <div ref={searchRef} style={{ position: 'relative' }}>
          <input className="stock-select-input"
            placeholder="搜索股票代码/名称添加自选股..."
            value={search}
            onChange={e => setSearch(e.target.value)}
            style={{ width: 260 }}
          />
          {addResults.length > 0 && search.trim() && (
            <div style={{
              ...
              {!addSearching && addResults.map(r => {
                ...
                return (
                  ...
                  >
                    ...
                    {alreadyAdded ? (
                      <span style={{ fontSize: 11, color: 'var(--fg-dim)', padding: '2px 8px' }}>✓ 已添加</span>
                    ) : (
                      <button
                        onClick={async () => {
                        ...
                        style={{
                          ...
                          border: '1px solid #3b82f6', whiteSpace: 'nowrap',
